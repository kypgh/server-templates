...
type AsyncHandler = (...args: Parameters<RequestHandler>) => Promise<ReturnType<RequestHandler>>;
export type RouteHandler = AsyncHandler | RequestHandler;
export type RouteConfig = {
  middleware?: {
    all?: (AsyncHandler | RequestHandler)[];
    get?: (AsyncHandler | RequestHandler)[];
    post?: (AsyncHandler | RequestHandler)[];
    put?: (AsyncHandler | RequestHandler)[];
    patch?: (AsyncHandler | RequestHandler)[];
    delete?: (AsyncHandler | RequestHandler)[];
  };
}
type CatchedHandler = (fn: (AsyncHandler | RequestHandler)) => AsyncHandler;
type ImportedFile = {
  GET?: RouteHandler;
  POST?: RouteHandler;
  PUT?: RouteHandler;
...
const catchAsyncError: CatchedHandler = (fn) => async (req, res, next) => {
  try {
...
const calcRoutePath = (_path: string, file: string, prefix: string, _originalPath: string): string => {
  let _fileName =
...
async function _folderRouter(
  ...
  for (const file of sortedFiles) {
    if (file.isDirectory()) {
      ...
      const endpoints: ImportedFile = await import(path.resolve(_path, file.name));
...
export default async function folderRouter(server: Express, _path: string, _prefix: string = "") {
